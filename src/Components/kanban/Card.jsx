...
export default function Card({ task, onOpenModal, onOpenAudit, isSearchActive = false, isSearchMatch = true }) {
  const { deleteTask, profile } = useTasks();

  const assignedUser = task.assigned_to || task.assignedTo || null;
  ...
  const getTimelineStatus = () => {
    if (!dueDateValue) return { isUrgent: false, isExpired: false, styles: 'border-l-indigo-500 text-slate-400 dark:text-slate-400' };
  ...
  const handleDragStart = (e) => {
    if (!canUpdate) {
      e.preventDefault();
      return alert('RBAC Access Violation: Workers can only update tasks assigned to their account.');
    }
  ...
  return (
    ...
    >
      ...
      <div className="mb-2 flex items-center justify-between gap-2 text-[9px] font-bold uppercase tracking-wide text-slate-400 dark:text-slate-500">
        <span className="rounded-md bg-slate-100 px-1.5 py-0.5 dark:bg-slate-800 dark:text-slate-300">
          {(task.status || 'backlog').replace('_', ' ')}
        </span>
        <span>Created {task.created_at ? new Date(task.created_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }) : 'recently'}</span>
      </div>
      ...
      <div className="pt-2 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-[10px] font-bold text-slate-400 dark:text-slate-500">
        <span className={`flex items-center gap-1 ${timeline.isUrgent ? 'text-amber-500 font-extrabold' : timeline.isExpired ? 'text-rose-500 font-extrabold' : ''}`}>
          ...
          {dueDateValue ? new Date(dueDateValue).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }) : 'No Timeline'}
        </span>
        ...
        <span className="flex items-center gap-1 bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded-md border border-slate-100 dark:border-slate-700 max-w-[120px] truncate">
          ...
          <span className="truncate">{assignedUser ? `@${assignedUser}` : 'Unassigned'}</span>
        </span>
      </div>
